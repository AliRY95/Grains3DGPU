...
__host__ __device__ Sphere::Sphere( double r )
: m_radius( r )
{}
...
// -----------------------------------------------------------------------------
// Sets the radius
__host__ __device__ void Sphere::setRadius( double r )
{
    m_radius = r;
}
...
__host__ __device__ ConvexType Sphere::getConvexType() const
{
    return ( SPHERE );
}
...
__host__ __device__ double Sphere::computeVolume() const
{
...
__host__ __device__ bool Sphere::computeInertia( double* inertia, 
                                              double* inertia_1 ) const
{
    inertia[1] = inertia[2] = inertia[4] = 0.;
    inertia[5] = inertia[3] = inertia[0] = 8. * M_PI * pow( m_radius, 5. ) / 15.;
...
__host__ __device__ double Sphere::computeCircumscribedRadius() const
{
    return ( m_radius );
}
...
__host__ __device__ Vec3f Sphere::computeAABB() const
{
...
__host__ __device__ Vec3d Sphere::support( Vec3d const& v ) const
{
    double norm = v.norm2();
    if ( norm < EPSILON3 )
        return ( Vec3d() );
