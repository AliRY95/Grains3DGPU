...
__HOSTDEVICE__ void Quaternion<T>::setQuaternion(Matrix3<T> const& rot)
{
    ...
    if(rot[Y][Y] > -rot[Z][Z] && rot[X][X] > -rot[Y][Y] && rot[X][X] > -rot[Z][Z])
    {
    ...
    // Case rotYY < - rotZZ, rotXX > rotYY and rotXX > rotZZ
    else if(rot[Y][Y] < -rot[Z][Z] && rot[X][X] > rot[Y][Y] && rot[X][X] > rot[Z][Z])
    {
        den      = pow(T(1) + rot[X][X] - rot[Y][Y] - rot[Z][Z], T(0.5));
...
// user.
template <typename T>
__HOSTDEVICE__ void Quaternion<T>::setRotFromTwoVectors(Vector3<T> const& u, Vector3<T> const& v)
{
...
{
    return (sqrt(m_vqt[X] * m_vqt[X] + m_vqt[Y] * m_vqt[Y] + m_vqt[Z] * m_vqt[Z] + m_w * m_w));
}
...
{
    return (m_vqt[X] * m_vqt[X] + m_vqt[Y] * m_vqt[Y] + m_vqt[Z] * m_vqt[Z] + m_w * m_w);
}
...
template <typename T>
__HOSTDEVICE__ Quaternion<T> Quaternion<T>::multLeftVec(Vector3<T> const& lhs) const
{
    T          tmp  = -lhs * m_vqt;
...
template <typename T>
__HOSTDEVICE__ Vector3<T> Quaternion<T>::multToVector3(Quaternion<T> const& q) const
{
...
// this x rhs^t
template <typename T>
__HOSTDEVICE__ Vector3<T> Quaternion<T>::multConjugateToVector3(Quaternion<T> const& q) const
{
    Vector3<T> vtmp(-(m_vqt ^ q.m_vqt) - (m_w * q.m_vqt) + (q.m_w * m_vqt));
    return (vtmp);
}
...
{
    Vector3<T> v_rotated
        = (m_w * m_w - m_vqt.norm2()) * v + T(2) * (v * m_vqt) * m_vqt + T(2) * m_w * (m_vqt ^ v);
    return (v_rotated);
}
...
#define X(T)                                                                               \
    template std::ostream& operator<< <T>(std::ostream & fileOut, Quaternion<T> const& q); \
                                                                                           \
