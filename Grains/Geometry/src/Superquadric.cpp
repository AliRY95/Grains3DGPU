...
template <typename T>
__HOSTDEVICE__ void Superquadric<T>::computeInertia(T (&inertia)[6],
                                                    T (&inertia_1)[6]) const
{
    T const eps1 = T(2) / m_n1;
    T const eps2 = T(2) / m_n2;
    ...
    T const prod2 = m_c * m_c * grainsBeta(T(0.5) * eps2, T(0.5) * eps2)
...
__HOSTDEVICE__ T Superquadric<T>::computeCircumscribedRadius() const
{
    ...
    else
    {
        ...
        T const beta
            = pow(gamma * m_c * m_c / (m_a * m_a), T(1) / (m_n1 - T(1)));
        T const xt
            = T(1)
              / pow(pow(T(1) + pow(alpha, m_n2), m_n1 / m_n2) + pow(beta, m_n1),
                    T(1) / m_n1);
...
__HOSTDEVICE__ Vector3<T> Superquadric<T>::support(const Vector3<T>& v) const
{
    ...
    if(abvx == T(0))
    {
        ...
        else
        {
            ...
            return (
                Vector3<T>(T(0), signy * m_b * yt, signz * alpha * m_c * yt));
        }
    }
    else
    {
        ...
        T const beta
            = gamma * pow(m_c / m_a * abvz / abvx, T(1) / (m_n1 - T(1)));
        T const xt
            = T(1) / pow(pow(temp, m_n1 / m_n2) + pow(beta, m_n1), T(1) / m_n1);
