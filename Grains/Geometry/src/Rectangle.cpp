...
template <typename T>
__HOSTDEVICE__ void Rectangle<T>::computeInertia(T (&inertia)[6],
                                                 T (&inertia_1)[6]) const
{
...
template <typename T>
__HOSTDEVICE__ Vector3<T> Rectangle<T>::support(Vector3<T> const& v) const
{
...
// Paraview format
template <typename T>
__HOST__ int Rectangle<T>::numberOfCells_PARAVIEW() const
{
    return (1);
}
...
__HOST__ std::list<Vector3<T>>
         Rectangle<T>::writePoints_PARAVIEW(Transform3<T> const& transform,
...
template <typename T>
__HOST__ void
    Rectangle<T>::writeConnection_PARAVIEW(std::list<int>& connectivity,
