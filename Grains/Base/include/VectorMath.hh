...
@param v the vector */
template <typename T>
__HOSTDEVICE__ static INLINE bool isApproxZero(Vector3<T> const& v, T tol = HIGHEPS<T>) noexcept
{
    T const* __RESTRICT__ buffer = v.getBuffer();
...
@param v the vector */
template <typename T>
__HOSTDEVICE__ static INLINE Vector3<T> operator*(T d, Vector3<T> const& v) noexcept
{
    T const* __RESTRICT__ buffer = v.getBuffer();
    T __RESTRICT__        out[3];
    for(unsigned int i = 0; i < 3; ++i)
...
@param v the vector */
template <typename T>
__HOSTDEVICE__ static INLINE Vector3<T> operator/(Vector3<T> const& v, T d) noexcept
{
    T const* __RESTRICT__ buffer = v.getBuffer();
    T __RESTRICT__        out[3];
    for(unsigned int i = 0; i < 3; ++i)
...
@param v2 2nd vector */
template <typename T>
__HOSTDEVICE__ static INLINE T operator*(Vector3<T> const& v1, Vector3<T> const& v2) noexcept
{
    T const* __RESTRICT__ b1  = v1.getBuffer();
    T const* __RESTRICT__ b2  = v2.getBuffer();
    T                     out = T(0);
