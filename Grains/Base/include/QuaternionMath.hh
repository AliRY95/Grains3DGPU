...
@param q the quaternion */
template <typename T>
__HOSTDEVICE__ static INLINE Quaternion<T> operator*(T d, Quaternion<T> const& q)
{
...
@param q the quaternion */
template <typename T>
__HOSTDEVICE__ static INLINE Quaternion<T> operator*(Quaternion<T> const& q, Vector3<T> const& v)
{
    T          tmp = -q.getVector() * v;
...
@param v the vector */
template <typename T>
__HOSTDEVICE__ static INLINE Quaternion<T> operator*(Vector3<T> const& v, Quaternion<T> const& q)
{
    T          tmp = -v * q.getVector();
