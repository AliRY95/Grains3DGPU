...
template <typename T>
std::vector<unsigned int> ComponentManagerCPU<T>::getRigidBodyIdObstacles() const
{
...
template <typename T>
void ComponentManagerCPU<T>::setRigidBodyIdObstacles(std::vector<unsigned int> const& id)
{
...
template <typename T>
void ComponentManagerCPU<T>::setTransformObstacles(std::vector<Transform3<T>> const& t)
{
...
template <typename T>
void ComponentManagerCPU<T>::initializeObstacles(std::vector<unsigned int>  numEachUniqueObstacles,
...
template <typename T>
void ComponentManagerCPU<T>::initializeParticles(std::vector<uint>          numEachUniqueParticles,
                                                 std::vector<Transform3<T>> initTr)
{
...
void ComponentManagerCPU<T>::updateLinks(LinkedCell<T> const* const* LC)
{
    ...
    (*LC)->computeLinearLinkedCellHashCPU(m_transform, m_nParticles, m_particleCellHash);
    ...
    ContactForceModel<T> const* const* CF)
{
    ...
    for(int pId = 0; pId < m_nParticles; pId++)
    {
        ...
        {
            RigidBody<T, T> const& rbB = *(obstacleRB[m_obstacleRigidBodyId[oId]]);
            ...
            if(ci.getOverlapDistance() < T(0))
            {
                ...
                CF[contactForceID]->computeForces(
                    ci, relVel, relAngVel, massA, rbB.getMass(), trA.getOrigin(), m_torce[pId]);
            }
        }
    }
}
                ...
                for(int i = -1; i < 2; i++)
                {
                    ...
                    for(auto id : m_cell[neighboringCellHash])
                    {
                        ...
                        if(secondaryId == particleId)
                            continue;
                        RigidBody<T, T> const& rbB = *(particleRB[m_rigidBodyId[secondaryId]]);
...
template <typename T>
void ComponentManagerCPU<T>::addExternalForces(RigidBody<T, T> const* const* particleRB,
                                               Vector3<T> const&             g)
{
    // #pragma omp parallel for
...
template <typename T>
void ComponentManagerCPU<T>::moveParticles(RigidBody<T, T> const* const*   particleRB,
                                           TimeIntegrator<T> const* const* TI)
{
    ...
    for(int pId = 0; pId < m_nParticles; pId++)
    {
        ...
        Kinematics<T> const& momentum
            = rb->computeMomentum(m_velocity[pId].getAngularComponent(), m_torce[pId], qRot);
        // Reset torces
        m_torce[pId].reset();
