...
template <typename T>
__GLOBAL__ void createLinkedCellOnDeviceKernel(
    T minX, T minY, T minZ, T maxX, T maxY, T maxZ, T size, LinkedCell<T>** LC, int* numCells)
{
    unsigned int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if(tid > 0)
        return;

    *LC       = new LinkedCell<T>(Vector3<T>(minX, minY, minZ), Vector3<T>(maxX, maxY, maxZ), size);
...
template <typename T>
__HOST__ int createLinkedCellOnDevice(Vector3<T> min, Vector3<T> max, T size, LinkedCell<T>** LC)
{
    ...
    createLinkedCellOnDeviceKernel<<<1, 1>>>(
        min[X], min[Y], min[Z], max[X], max[Y], max[Z], size, LC, d_numCells);

    // copying the variable to host
    cudaMemcpy(&h_numCells, d_numCells, sizeof(int), cudaMemcpyDeviceToHost);
...
#define X(T)                                                         \
    template __HOST__ int createLinkedCellOnDevice(                  \
        Vector3<T> min, Vector3<T> max, T size, LinkedCell<T>** LC); \
                                                                     \
