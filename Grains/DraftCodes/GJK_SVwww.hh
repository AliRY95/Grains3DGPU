 ...
 */
__host__ __device__ inline unsigned int compareSigns( double a, double b )
{
 ...
 */
__host__ __device__ inline void s1d(vec3d* W, unsigned int& W_used, double* lambdas)
{
// Identify the appropriate indices
constexpr unsigned int max_num_points = 4;
bool s1_set = false;
 ...
 * combination of W represents the point of minimum norm.
 */
__host__ __device__ inline void s2d(vec3d* W, unsigned int& W_used, double* lambdas)
{
...
vec3d n = ( W[point1_idx] - W[point0_idx] ) ^ ( W[point2_idx] - W[point0_idx] );
vec3d p0 = ( W[point0_idx] * n ) / ( n * n ) * n;
...
double mu_max = (W[point1_idx][1] * W[point2_idx][2] + W[point0_idx][1] * W[point1_idx][2]
...
double mu = (W[point1_idx][2] * W[point0_idx][0] + W[point2_idx][2] * W[point1_idx][0]
...
else
{
    ...
    for (unsigned int j = 0; j < num_points; ++j)
    {
        if (!sign_comparisons[j])
        {
            ...
            double new_lambdas[max_num_points] = {0};

            s1d( W, new_used, new_lambdas );
            ...
            double d_star = dot(new_point, new_point);
            if ( d_star < d )
            {
                new_W_used = new_used;
                d = d_star;
 ...
 * combination of W represents the point of minimum norm.
 */
__host__ __device__ inline void s3d(vec3d* W, unsigned int& W_used, double* lambdas)
    {
 ...
 * combination of W represents the point of minimum norm.
 */
HOSTDEVICE inline void sv_subalgorithm(vec3d* W, unsigned int& W_used, double* lambdas)
    {
    ...
    if (num_used == 1)
        ...
        {
        s1d(W, W_used, lambdas);
        }
    else if (num_used == 3)
        {
        s2d(W, W_used, lambdas);
        }
    else if (ndim == 3)
        {
