...
class Transform3
{
  ...
  public:
    ...
    __host__ __device__ Transform3( T def = T() );
    ...
    __host__ __device__ Transform3( T x, T y, T z );
    ...
    __host__ __device__ Transform3( T const t[12] );

    /** @brief Copy constructor
    @param t the transformation to be copied */
    __host__ __device__ Transform3( Transform3<T> const& t );
    ...
    The 1D array must be organized as: 0=Mxx, 1=Mxy, 2=Mxz, 3=Myx, 4=Myy, 5=Myz,
    6=Mzx, 7=Mzy, 8=Mzz, 9=Ox, 10=Oy, 11=Oz 
    @param t the 1D array of values containing the tranformation coefficients */
    __host__ __device__ void setValue( T const t[12] );
    ...
    __host__ __device__ void setToInverseTransform( Transform3<T> const& t );
    ...
    __host__ __device__ void setToTransformsComposition( Transform3<T> const& t1, 
                                                         Transform3<T> const& t2 );

    /** @brief Returns the orientation of the transformation */
    __host__ __device__ Matrix3<T> getBasis() const;

    /** @brief Returns the origin of the transformation */
    __host__ __device__ Vector3<T> getOrigin() const;
    ...
    __host__ __device__ void composeWithScaling( T x, T y, T z );  
    ...
    __host__ __device__ void composeLeftByRotation( Transform3<T> const& t );  

    /** @brief Composition on the left by a translation:
    this = trans(vector) o this (this first followed by trans(vector))
    @param v translation vector */
    __host__ __device__ void composeLeftByTranslation( Vector3<T> const& v );
    ...
    __host__ __device__ void composeLeftByTransform( Transform3<T> const& t );
